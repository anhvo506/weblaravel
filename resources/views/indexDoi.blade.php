  ...
  <nav class="navbar bg-body-tertiary fixed-top">
    <div class="container-fluid content-adjust" id="main-container">
      ...
      <div class="container-fluid mt-4 ml-3">
        ...
        <a href="{{ route('showCreateForm') }}" class="border-shadow">

          <button type="button" class="btn btn-primary">Tạo mới đội</button>
          </a>
        </div>
        ...
        <table class="table table-striped table-hover">
          ...
          <tr>
                    ...
                    <td>{{ $doi->ten_doi }}</td>
                      ...
                      <td>
                        @foreach($doi->nhoms as $nhom)
                            <p>Nhóm {{ $nhom->thu_tu_nhom }}: {{ $nhom->ten_nhom }}</p>                          
                        @endforeach
                    </td>
                    <td>
                      <div class="d-inline-block">
                        <button type="button" class="btn btn-primary">
                        <a href="{{ route('showEditDoi', $doi->id) }}"
                          style="color: white; text-decoration: none;">Sửa</a>
                        </button>
                      </div>
                      <div class="d-inline-block">
                        <form action="{{ route('deleteDoi', $doi->id) }}" method="POST"
                        onsubmit="return confirm('Bạn có chắc chắn muốn xóa tuyến này?');" style="display: inline;">
                          @csrf
                          @method('DELETE')
