...
<body>
  <nav class="navbar bg-body-tertiary fixed-top">
    ...
    <a href="{{ route('showCreateTram') }}" class="border-shadow">
        <button type="button" class="btn btn-primary">Tạo mới</button>
    </a>
</div>

<form method="GET" action="{{ route('tram') }}">
    ...
    <div class="mb-3">
        ...
        <select class="form-select form-select-sm form_select" id="phat_tuyen_id" name="phat_tuyen_id" aria-label="Small select example">
            ...
            @foreach($phatTuyens as $phatTuyen)
                <option value="{{ $phatTuyen->id }}" {{ $phatTuyenId == $phatTuyen->id ? 'selected' : '' }}>
                    {{ $phatTuyen->ten_tuyen }}
                </option>
            @endforeach
        </select>
    ...
    <div class="mb-3">
        ...
        <select class="form-select form-select-sm form_select" id="duong_day_id" name="duong_day_id" aria-label="Small select example">
    ...
    <tbody>
        @if($tramBienAps->isNotEmpty())
            @foreach($tramBienAps as $tramBienAp)
                <tr>
                <td>{{ $tramBienAp->duongDayTrungThe->tuyen->ten_tuyen ?? 'N/A' }}</td>
                <td>{{ $tramBienAp->duongDayTrungThe->ten_duong_day ?? 'N/A' }}</td>
                    <td>{{ $tramBienAp->ten_tram }}</td>
                    <td>{{ $tramBienAp->dung_luong }}</td>
                    <td>Nhóm {{ $tramBienAp->group->thu_tu_nhom ?? '' }}: {{ $tramBienAp->group->ten_nhom ?? 'Chưa phân công' }}</td> <!-- Hiển thị tên nhóm -->
                    
                    <td>
                        <a href="{{ route('assignGroup', $tramBienAp->id) }}" class="btn btn-warning">
                            Phân Công
                        </a>
                    </td>
                    <td>
                        <button type="button" class="btn btn-primary">
                            <a href="{{ route('showFormEditTram', $tramBienAp->id) }}" style="color: white; text-decoration: none;">Sửa</a>
                        </button>
                        <form action="{{ route('deleteTram', $tramBienAp->id) }}" method="POST" style="display: inline;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-danger" onclick="return confirm('Bạn có chắc chắn muốn xóa trạm biến áp này?')">Xóa</button>
                            </form>
                    </td>
                </tr>
            @endforeach
        @else
            <tr>
                <td colspan="6" class="text-center">Không có trạm biến áp nào</td>
            </tr>
        @endif
    </tbody>
</table>
