  ...
  <nav class="navbar bg-body-tertiary fixed-top">
    <div class="container-fluid content-adjust" id="main-container">
      ...
      <div class="container-fluid mt-4 ml-3">
        ...
        <a href="{{ route('showCreateNhanVien') }}" class="border-shadow">
    ...
    <tbody>
        @foreach($nhanViens as $nhanVien)
            <tr>
                ...
                <td>{{ optional($nhanVien->user)->email }}</td>
                <td>
                    @if($nhanVien->permission)
                        <span class="badge bg-secondary">{{ $nhanVien->permission->name }}</span>
                    @else
                        Không có quyền
                    @endif
                </td>
                ...
                <div class="d-inline-block">
                    <button type="button" class="btn btn-primary">
                        <a href="{{ route('showFormEditNhanVien', ['id' => $nhanVien->id]) }}">Sửa</a>
                    </button>
                 </div>
                 <div class="d-inline-block">
                    <form action="{{ route('deleteNhanVien', $nhanVien->id) }}" method="POST" onsubmit="return confirm('Bạn có chắc chắn muốn xóa không?');">
                        @csrf
                        @method('DELETE')
