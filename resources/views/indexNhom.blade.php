  ...
  <nav class="navbar bg-body-tertiary fixed-top">
    <div class="container-fluid content-adjust" id="main-container">
      ...
      <div class="container-fluid mt-4 ml-3">
        ...
        <a href="{{ route('showCreateNhom') }}" class="border-shadow">
    ...
    <tbody>
        @foreach($nhoms as $nhom)
            <tr>
                <td><p>Nhóm {{ $nhom->thu_tu_nhom }}: {{ $nhom->ten_nhom }}</p>      </td>
                <td>
                    @if($nhom->nhanViens->isEmpty())
                        Không có nhân viên
                    @else
                ...
                <td>
                <a href="{{ route('showFormEditNhom', $nhom->id) }}" class="btn btn-primary">Sửa</a>
                    <form action="{{ route('deleteNhom', $nhom->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Bạn có chắc chắn muốn xóa không?');">
                        @csrf
                        @method('DELETE')
