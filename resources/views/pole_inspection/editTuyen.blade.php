  ...
  <nav class="navbar bg-body-tertiary fixed-top">
    <div class="container-fluid content-adjust" id="main-container">
      ...
      <div class="container-fluid mt-4 ml-3">
        ...
        <form action="{{ route('updateTuyen', $tuyen->id) }}" method="POST">
          @csrf
          <div class="mb-3">
            <label for="ten_tuyen" class="form-label">Tên Tuyến</label>
            <input type="text" name="ten_tuyen" class="form-control" id="ten_tuyen" value="{{ $tuyen->ten_tuyen }}"
              required>
          </div>
          <div class="mb-3">
          ...
          <option value="{{ $donVi->id }}" {{ $tuyen->id_don_vi == $donVi->id ? 'selected' : '' }}>
          {{ $donVi->ten_don_vi }}
          </option>
        @endforeach
            </select>
