...
<body>
  <nav class="navbar bg-body-tertiary fixed-top">
    <div class="container-fluid content-adjust" id="main-container">
      ...
      <form action="{{ route('createDoi') }}" method="POST" id="update_user">
    @csrf
    <div class="new_user">
        <div class="mb-3">
            <label for="ma_doi" class="form-label">Mã đội</label>
            <input type="text" name="ma_doi" class="form-control" placeholder="Nhập mã đội" required>
        </div>
        <div class="mb-3">
            <label for="ten_doi" class="form-label">Tên đội</label>
            <input type="text" name="ten_doi" class="form-control" placeholder="Nhập tên đội" required>
        </div>
        
        <div class="mb-3">
            <label for="nhom_ids" class="form-label">Chọn nhóm</label>
            <div id="nhom_ids">
                @foreach($nhoms as $nhom)
                    <div class="form-check">
                        <input type="checkbox" class="form-check-input" name="nhom_ids[]" value="{{ $nhom->id }}" id="nhom_{{ $nhom->id }}">
