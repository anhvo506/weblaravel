...
<body>
  <nav class="navbar bg-body-tertiary fixed-top">
    <div class="container-fluid content-adjust" id="main-container">
      ...
      <form action="{{ route('createDuongDay') }}" method="POST" id="update_user">
    @csrf
    <div class="new_user">
        <div class="mb-3">
            <label for="ten_duong_day" class="form-label">Tên Đường Dây</label>
            <input type="text" name="ten_duong_day" class="form-control" id="ten_duong_day" required>
        </div>
        <div class="mb-3">
            <label for="tu_vi_tri_tru" class="form-label">Từ Vị Trí Trụ</label>
            <input type="text" name="tu_vi_tri_tru" class="form-control" id="tu_vi_tri_tru" required>
        </div>
        <div class="mb-3">
            <label for="den_vi_tri_tru" class="form-label">Đến Vị Trí Trụ</label>
            <input type="text" name="den_vi_tri_tru" class="form-control" id="den_vi_tri_tru" required>
        </div>
        <div class="mb-3">
            <label for="chieu_dai" class="form-label">Chiều Dài</label>
            <input type="text" name="chieu_dai" class="form-control" id="chieu_dai" required>
        </div>
        <div class="mb-3">
            <label for="phat_tuyen_id" class="form-label">Chọn Phát Tuyến</label>
            <select class="form-select" name="phat_tuyen_id" id="phat_tuyen_id" required>
                <option value="">Chọn Phát Tuyến</option>
                @foreach($phatTuyens as $phatTuyen)
                    <option value="{{ $phatTuyen->id }}">{{ $phatTuyen->ten_tuyen }}</option>
                @endforeach
            </select>
        </div>
        <div class="d-flex">
