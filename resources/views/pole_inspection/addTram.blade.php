...
<body>
 ...
 <!-- Form lọc tuyến (GET) -->
<form action="{{ route('showCreateTram') }}" method="GET">
    <div class="mb-3">
        ...
        <select class="form-select" id="tuyen_id" name="tuyen_id" onchange="this.form.submit()">
            ...
            @foreach($tuyens as $tuyen)
                <option value="{{ $tuyen->id }}" {{ $selectedTuyenId == $tuyen->id ? 'selected' : '' }}>
                    {{ $tuyen->ten_tuyen }}
                </option>
            @endforeach
        </select>
...
<!-- Form tạo mới trạm (POST) -->
<form action="{{ route('createTram') }}" method="POST">
    @csrf
    <div class="mb-3">
        <label for="duong_day_id" class="form-label">Chọn Đường Dây</label>
        <select class="form-select" id="duong_day_id" name="duong_day_id" required>
            <option value="">Chọn Đường Dây</option>
            @foreach($duongDays as $duongDay)
                <option value="{{ $duongDay->id }}" {{ $selectedDuongDayId == $duongDay->id ? 'selected' : '' }}>
    ...
    <div class="mb-3">
        <label for="ten_tram" class="form-label">Tên Trạm</label>
    ...
    <input type="hidden" name="tuyen_id" value="{{ $selectedTuyenId }}">
