...
<body>
  <nav class="navbar bg-body-tertiary fixed-top">
    ...
    overflow-y: auto;">
        ...
        <form action="{{ route('updateNhanVien', $nhanVien->id) }}" method="POST">
    @csrf
    @method('PUT')

    <div class="mb-3">
        <label for="name" class="form-label">Tên</label>
        <input type="text" class="form-control" id="name" name="name" value="{{ old('name', $nhanVien->user->name) }}" required>
    </div>

    <div class="mb-3">
        <label for="email" class="form-label">Email</label>
        <input type="email" class="form-control" id="email" name="email" value="{{ old('email', $nhanVien->user->email) }}" required>
    </div>
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control" id="ten_nhan_vien" name="ten_nhan_vien" value="{{ old('ten_nhan_vien', $nhanVien->ten_nhan_vien) }}" required>
    </div>

    <div class="mb-3">
        <label for="ma_nhan_vien" class="form-label">Mã nhân viên</label>
        <input type="text" class="form-control" id="ma_nhan_vien" name="ma_nhan_vien" value="{{ old('ma_nhan_vien', $nhanVien->ma_nhan_vien) }}" required>
    </div>

    <div class="mb-3">
        <label for="bo_phan" class="form-label">Bộ phận</label>
        <input type="text" class="form-control" id="bo_phan" name="bo_phan" value="{{ old('bo_phan', $nhanVien->bo_phan) }}" required>
    </div>

    <div class="mb-3">
        <label for="chuc_danh" class="form-label">Chức danh</label>
        <input type="text" class="form-control" id="chuc_danh" name="chuc_danh" value="{{ old('chuc_danh', $nhanVien->chuc_danh) }}" required>
    </div>

    <div class="mb-3">
        <label for="bac_tho" class="form-label">Bậc thợ</label>
        <input type="text" class="form-control" id="bac_tho" name="bac_tho" value="{{ old('bac_tho', $nhanVien->bac_tho) }}" required>
    </div>

    <div class="mb-3">
        <label for="bac_AT" class="form-label">Bậc AT</label>
        <input type="text" class="form-control" id="bac_AT" name="bac_AT" value="{{ old('bac_AT', $nhanVien->bac_AT) }}" required>
    </div>

    <div class="mb-3">
        <label for="id_permission" class="form-label">Quyền</label>
        <select class="form-select" id="id_permission" name="id_permission">
            @foreach($permissions as $permission)
                <option value="{{ $permission->id }}" {{ $nhanVien->permission_id == $permission->id ? 'selected' : '' }}>
                    {{ $permission->name }}
                </option>
            @endforeach
        </select>
