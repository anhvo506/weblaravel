...
<body>
  <nav class="navbar bg-body-tertiary fixed-top">
    ...
    overflow-y: auto;">
        ...
        <form method="POST" action="{{ route('updateTram', $tramBienAp->id) }}">
        @csrf
        @method('PUT')

        <div class="mb-3">
            <label for="ten_tram" class="form-label">Tên Trạm</label>
            <input type="text" class="form-control" id="ten_tram" name="ten_tram" value="{{ old('ten_tram', $tramBienAp->ten_tram) }}" required>
        </div>

        <div class="mb-3">
            <label for="dung_luong" class="form-label">Dung Lượng</label>
            <input type="number" class="form-control" id="dung_luong" name="dung_luong" value="{{ old('dung_luong', $tramBienAp->dung_luong) }}" required>
        </div>

        <button type="submit" class="btn btn-primary">Cập Nhật</button>
        <a href="{{ route('tram') }}" class="btn btn-secondary">Quay Lại</a>
    </form>
