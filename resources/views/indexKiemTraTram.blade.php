...
<form method="GET" action="{{ route('kiemtratram') }}">
    <!-- Chọn Tuyến -->
    <div class="mb-3">
    ...
    <div class="mb-3">
        ...
        <select class="form-select" id="tram_bien_ap_id" name="tram_bien_ap_id" onchange="this.form.submit()">
...
<div class="mt-3">
    <h4>Thông Tin Kiểm Tra Trạm Biến Áp</h4>
    <div class="table-responsive">
        <table class="table table-striped table-hover table-bordered">
            ...
            <tbody>
                @if($kiemTraTBA)
                    <tr>
                        <td>{{ $kiemTraTBA->gio_kiem_tra }}</td>
                        <td>{{ $kiemTraTBA->hien_tuong_bat_thuong }}</td>
                        <td>{{ $kiemTraTBA->ton_tai_da_xu_ly }}</td>
                        <td>{{ $kiemTraTBA->bien_phap_de_nghi }}</td>
                        <td>
                            <!-- Nút Sửa -->
                            <a href="{{ route('showFormEditKiemTraTBA', $kiemTraTBA->id) }}" class="btn btn-primary btn-sm">Sửa</a>
                            
                            <!-- Nút Xóa -->
                            <form action="{{ route('deleteKiemTraTBA', $kiemTraTBA->id) }}" method="POST" style="display: inline;">
                                @csrf
                                @method('DELETE')
